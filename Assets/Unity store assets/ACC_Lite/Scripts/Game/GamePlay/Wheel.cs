...
public struct Wheel
{
	...
	/// </summary>
	public void UpdateVisual (bool hover,bool is_ai,bool is_popo,drift_management drift,float speed)
	{
		UpdateTransform ();
		
		if (WheelCollider.isGrounded && CurrentMaxSlip > SlipForGenerateParticle)
		{
			if (drift.isEventActive&&speed>10.0&& !is_ai&&!is_popo)
			{
				drift.AddDriftPoints();
			}
            ...
            if (!hover)
            {
				...
				if (Trail == null)
				{
					...
					Trail = FXController.GetTrail(HitPoint,hover);
