...
public class CarController :MonoBehaviour
{
	...
	public bool Is_AI;
	public bool Is_Hover;
	public bool Is_police;
	public drift_management drift_Management;
	...
	private void Awake ()
	{
		...
		drift_Management = GameObject.Find("GameController").GetComponent<drift_management>();

		foreach( Wheel wheel in Wheels)
		{
			if(Is_Hover)
			{
				wheel.WheelView.GetComponentInChildren<MeshRenderer>().enabled = false;

			}
			else
			{
                wheel.WheelView.GetComponentInChildren<MeshRenderer>().enabled = true;
	...
	private void Update ()
	{
		...
		{
			Wheels[i].UpdateVisual(Is_Hover,Is_AI, Is_police, drift_Management,CurrentSpeed);
		}
		...
		if(Is_AI)
		{
            ...
            if (Vector3.Distance(transform.position, ai.raceManager.waypoints[ai.currentWaypointIndex].transform.position) < ai.targetRadius)
              {

                UpdateControls(ai.steeringAngle,-0.8f,true);
    ...
    public void reset_car(Transform position)
    {
    ...
    /// <summary>
    /// UpdateControls all helpers logic.
    /// </summary>
